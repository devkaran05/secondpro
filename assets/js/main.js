...
document.addEventListener("DOMContentLoaded", function() {
  function hello() {
    ...
    var list = document.querySelectorAll(".nav__item");

    list.forEach((item) => {
      var link = item.querySelector(".nav__item-link");
      var pageName = link.getAttribute("href").split(".")[0];
...
document.addEventListener("click", function (event) {
  const dropdowns = document.querySelectorAll(".unique-dropdown .dropdown");

  dropdowns.forEach(dropdown => {
      const wrapper = dropdown.closest(".dropdown-wrapper");
      if (wrapper && !wrapper.contains(event.target)) {
          dropdown.checked = false; // Uncheck if clicked outside
      }
  });
});
